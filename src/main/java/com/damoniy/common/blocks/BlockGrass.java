...
import com.damoniy.common.blocks.enums.EnumGrass;
...
public class BlockGrass extends BlockSpreadableSnowyDirt implements IGrowable {

	public BlockGrass(String name) {
		super(Properties.create(Material.ORGANIC).hardnessAndResistance(0.6F).sound(SoundType.PLANT).tickRandomly(),
	...
	public BlockGrass registerVariants() {
		for (EnumGrass type : EnumGrass.values()) {
			new BlockGrass("block_grass_" + type.getName());
		}
		return this;
	}
			...
			while (true) {
				if (j >= i / 16) {
					...
					if (p_225535_2_.nextInt(8) == 0) {
						...
						blockstate1 = ((FlowersFeature) configuredfeature.feature).getFlowerToPlace(p_225535_2_,
								blockpos1, configuredfeature.config);
					} else {
				...
				blockpos1 = blockpos1.add(p_225535_2_.nextInt(3) - 1,
						(p_225535_2_.nextInt(3) - 1) * p_225535_2_.nextInt(3) / 2, p_225535_2_.nextInt(3) - 1);
				if (p_225535_1_.getBlockState(blockpos1.down()).getBlock() != this
						|| p_225535_1_.getBlockState(blockpos1).isCollisionShapeOpaque(p_225535_1_, blockpos1)) {
					break;
				}
