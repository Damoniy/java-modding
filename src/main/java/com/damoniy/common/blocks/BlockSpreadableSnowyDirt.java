...
public abstract class BlockSpreadableSnowyDirt extends BlockSnowyDirt {
	protected BlockSpreadableSnowyDirt(Properties props, String name, ItemGroup itemGroupIn) {
		super(props, name, itemGroupIn);
	}

	public BlockSpreadableSnowyDirt() {
		super();
	}
		...
		if (!func_220257_b(state, worldIn, pos)) {
			if (!worldIn.isAreaLoaded(pos, 3))
				return; // Forge: prevent loading unloaded chunks when checking neighbor's light and
						// spreading
			worldIn.setBlockState(pos, BlockHandler.dirt.getDefaultState());
		} else {
			if (worldIn.getLight(pos.up()) >= 9) {
				...
				for (int i = 0; i < 4; ++i) {
					...
					if (worldIn.getBlockState(blockpos).getBlock() == BlockHandler.dirt
